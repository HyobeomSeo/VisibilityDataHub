...
@Component
public class InputDataConsumer {

	@KafkaListener(topics = "${topic.name.datainput}", containerFactory = "inputDataKafkaListenerContainerFactory")
	public void inputDataListener(InputData inputData, Acknowledgment ack) {
		
		try {
            System.out.println("[InputDataConsumer] Recieved inputData: " + inputData.toString());
